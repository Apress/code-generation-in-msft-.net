...
<!--
  ...
  ====================================================================
  Summary:  Creates the Update Stored Procedure for TSQL-->
...
SET NOCOUNT ON 
	...
	UPDATE [<xsl:value-of select="$origtablename"/>]
		SET
			<xsl:apply-templates select="orm:RunSPParam[not(@IsAutoIncrement='true')]" />
		WHERE <xsl:value-of select="ancestor::orm:SPSet/orm:WhereClause/@Clause" />
...
<xsl:template match="orm:RunSPParam">
