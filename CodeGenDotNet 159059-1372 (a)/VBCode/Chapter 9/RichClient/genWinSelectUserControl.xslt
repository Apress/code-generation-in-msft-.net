...
<!--
  ...
  ====================================================================
  Summary: Generates the plumbing class for the select user control  -->
...
<xsl:variable name="baseformname" select="'WinSupport.BaseSelectUserControl'" />
...
<xsl:template name="RestOfForm">
	...
	Private mList As <xsl:value-of select="$singularobject"/>List
	
	Public Overrides Function GetList() As CSLA.ReadOnlyCollectionBase
		mList = <xsl:value-of select="$objectname"/>List.Get<xsl:value-of select="$singularobject"/>List
		Return mList
	End Function
	...
	Protected Overrides ReadOnly Property GetResult() As Object
	   Get
		   If dgDisplay.CurrentRowIndex >= 0 Then
			   Return mList(dgDisplay.CurrentRowIndex).<xsl:value-of select="orm:Property[@IsPrimaryKey='true'][1]/@Name"/>
		   Else
			   Return <xsl:value-of select="orm:Property[@IsPrimaryKey='true'][1]/@Empty"/>
		   End If
		End Get
	End Property
