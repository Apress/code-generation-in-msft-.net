	   ...
	   <xsl:for-each select="$properties">
	      <xsl:choose>
	         <xsl:when test="@IsLookup='true'">
		         ...
		         <xsl:variable name="lookupname" select="@LookupName"/>
               <!-- This cheats and uses the child key name. That's not right, but I'd like to fix it in WinInputTransform. -->
               <xsl:variable name="lookupfield" select="@Name"/>
         cbo<xsl:value-of select="$ctlname"/>.DataSource = <xsl:value-of select="@LookupSingular"/>List.Get<xsl:value-of select="@LookupSingular"/>List();<xsl:text/>
         cbo<xsl:value-of select="$ctlname"/>.ValueMember = "<xsl:value-of select="$lookupfield" />";
