...
<!--
  ...
  ====================================================================
  Summary: Generates the handcrafted class for business objects !-->
...
{
   [Serializable()]
   <xsl:if test="@Root='true'">[Root()]
   </xsl:if>
   public class <xsl:value-of select="$Name"/> : gen<xsl:value-of select="$Name"/>
   {  
      internal <xsl:value-of select="$Name"/>() : base()
      {
      }
