...
#Region "Protected and Friend Methods and Properties"
   ...
   Protected Overrides Function MakeName( _
      ...
      Dim nsmgr As Xml.XmlNamespaceManager = GetNameSpaceManager(node.OwnerDocument, "kg")
      Dim nodeStandard As Xml.XmlNode = node.SelectSingleNode("kg:Standard", nsmgr)
      Dim nodename As String = KADGen.Utility.Tools.SpaceAtCaps(KADGen.Utility.Tools.StripNamespacePrefix(node.Name))
      Dim name As String
      If Not nodeStandard Is Nothing Then
         name = KADGen.Utility.Tools.GetAttributeOrEmpty(nodeStandard, "Name")
         If name.Trim.Length > 0 Then
            nodename &= "  (" & KADGen.Utility.Tools.SpaceAtCaps(name) & ")"
         End If
      End If
      If name Is Nothing OrElse name.Trim.Length = 0 Then
         nodename = KADGen.Utility.Tools.StripNamespacePrefix(MyBase.MakeName(node))
      End If
      Return nodename
   End Function
